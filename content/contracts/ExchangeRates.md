...
This does not turn off any functionality in the exchange rate contract, but is used by [`Oikos`](Oikos.md) to disable [currency exchanges](Oikos.md#_internalexchange) while prices are being updated to protect against oracle front running. The lock is released when [rate updates have completed](#internalupdaterates).

**Source:** [ExchangeRates.sol](https://github.com/Synthetixio/oikos/blob/master/contracts/ExchangeRates.sol)
...
??? example "Details"
    ...
    - [`Oikos`](Oikos.md): the value of tokens is used to in order to facilitate exchange between them, and to ensure exchanges cannot occur while price updates and being made or if a particular exchange rate is stale.
...
The duration after which a rate will be considered out of date. Synth exchange and other price-sensitive transactions in the [`Oikos`](Oikos.md) contract will not operate if a relevant rate is stale.
