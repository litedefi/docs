...
**Source:** [RewardEscrow.sol](https://github.com/Synthetixio/oikos/blob/master/contracts/RewardEscrow.sol)
...
The address of the main [`Oikos`](Oikos.md) contract.
...
Initialises the [`Oikos`](Oikos.md) and [`FeePool`](FeePool.md) contract addresses, and the inherited [`Owned`](Owned.md) instance.
...
Sets the address of the [`Oikos`](Oikos.md) contract, so that escrowed OKS can be transferred to accounts claiming them.
