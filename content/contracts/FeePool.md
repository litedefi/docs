...
**Source:** [FeePool.sol](https://github.com/Synthetixio/oikos/blob/master/contracts/FeePool.sol)
...
??? example "Details"
    ...
    - [`Oikos`](Oikos.md): The fee pool needs the Oikos address for a onlySynthetix modifer for storing a minters issue and burn events to track their debt % of the system.
    - [`SynthetixState`](SynthetixState.md): The fee pool retrieves the global issuance ratio, and queries the debt ledger directly from the Oikos state contract.
...
**Value:** [`0xfeEFEEfeefEeFeefEEFEEfEeFeefEEFeeFEEFEeF`](https://tronscan.io/#/address/0xfeEFEEfeefEeFeefEEFEEfEeFeefEEFeeFEEFEeF)
...
The main [`Oikos`](Oikos.md) contract.
...
Computes the quantity received if a quantity of Synths is exchanged into another flavour. The amount received is the quantity sent minus the [exchange fee](#exchangefeeincurred), as per the logic in [`Oikos._internalExchange`](Oikos.md#_internalexchange).
...
This function merely emits an event and passes through to [`FeePoolState.appendAccountIssuanceRecord`](FeePoolState.md#appendAccountIssuanceRecord) and is itself only invoked by [`Oikos._appendAccountIssuanceRecord`](Oikos.md#_appendaccountissuancerecord).
...
Allows the [`Oikos._internalExchange`](Oikos.md#_internalexchange) function to record that a fee was paid whenever an exchange between Synth flavours occurs.
...
See [`Oikos._addToDebtRegister`](Oikos.md#_addToDebtRegister) for details of the debt ownership percentage adjustment.
