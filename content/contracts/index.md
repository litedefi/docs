...
| [`SynthetixState`](SynthetixState.md) | An auxiliary state contract that sits alongside Oikos, which tracks current issuer data and the debt ledger. |
...
!!! example "Responsibilities"
    ...
    _ Distributes inflationary rewards to different recipients in the proportions specified by the protocol; that is for staking versus providing Oikos Swap liquidity.
...
| [`SynthetixEscrow`](SynthetixEscrow.md)         | Holds the escrowed balances of OKS from the original token sale.                                                  |
...
The [`Depot`](Depot.md) is a vendor contract that allows users to exchange their TRX for sUSD or OKS, or their sUSD for OKS. It also allows users to deposit Synths to be sold in exchange for TRX.
